...
CREATE TABLE `SpotifyClone`.`plan` (
  ...
  `plan_cost` DECIMAL(7,2) NOT NULL
) ENGINE = InnoDB;

INSERT INTO SpotifyClone.plan (plan_name, plan_cost)
VALUES ('gratuito', 0),
       ('familiar', 7.99),
       ('universitário', 5.99),
       ('pessoal', 6.99);
...
INSERT INTO SpotifyClone.user (user_name, age, signature_on, plan_id)
...
INSERT INTO SpotifyClone.song (song_name, duration_seconds, album_id)
...
CREATE TABLE `SpotifyClone`.`reproduction_history` (
  ...
  `reproduction` DATETIME NOT NULL,
   FOREIGN KEY (`user_id`) 
      REFERENCES SpotifyClone.user(`user_id`),
   FOREIGN KEY (song_id) 
...
INSERT INTO SpotifyClone.reproduction_history (`user_id`, song_id, reproduction)
